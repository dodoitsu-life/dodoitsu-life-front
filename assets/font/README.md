...
- IBMPlexSansJP-Bold.ttf (圧縮前)
- sans-jp.ttf (サブセット後)
- sans-jp.woff (woff 変換後)

## 圧縮方法

以下の手順でフォントファイルを圧縮します。

1. [サブセットメーカー](https://opentype.jp/bin/subsetfont320.msi)をダウンロードし、インストールします。
2. サブセットメーカーを起動し、圧縮したいフォントファイル（この場合は IBMPlexSansJP-Bold.ttf）を選択します。
3. 「サブセットする文字列」に以下の文字列を入力します。この文字列は適宜アップデートすることが可能です。
...
4. 「サブセット作成」ボタンをクリックして、サブセット化されたフォントファイル（sans-jp.ttf）を生成します。
5. 生成された sans-jp.ttf を woff 形式に変換します。これには[TTF WOFF コンバーター](https://convertio.co/ja/ttf-woff/)を使用します。変換後のファイル名は sans-jp.woff とします。

以上の手順で、フォントファイルがサブセット化され、woff 形式に変換されます。
